...
import { breadCrumbRoutes, slugify } from 'core/helpers'
...
export default {
    ...
    let setbrcmb = (path) => {
      if (path.findIndex(itm => {
        ...
        path.push({
          ...
          name: context.rootState.category.current.labels.ru_RU
      ...
      if (product.categories && product.categories.length > 0) {
        subloaders.push(
          context.dispatch('category/list', {}, { root: true }).then((categories) => {
            for (let cat of product.categories.reverse()) {
              let category = categories.items.find((itm) => { return itm['code'] === cat })
              if (category) {
                context.dispatch('category/single', { key: 'code', value: category.code }, { root: true }).then((category) => { // this sets up category path and current category
  ...
  list (context, { query, start = 0, size = 50, entityType = 'product', sort = '', cacheByKey = 'sku', prefetchGroupProducts = true, updateState = true, meta = {} }) {
    return quickSearchByQuery({ query, start, size, entityType, sort: sort }).then((resp) => {
      return calculateTaxes(resp.items, context).then((updatedProducts) => {
        ...
        for (let product of resp.items) {
          if (!product.slug || product.slug === '') {
            product.slug = product.label.hasOwnProperty('<all_channels>') ? slugify(product.label['<all_channels>']['<all_locales>']) : product.sku
          }
        }
