...
import watched from './modules/watched-products'
import themeModules from 'theme/store'
import cmspages from './modules/cmspages'
import cmsblocks from './modules/cmsblocks'
...
Vue.prototype.$db = {
  ...
  watchedCollection: new UniversalStorage(localForage.createInstance({
    name: 'shop',
    storeName: 'watched'
  })),
...
const plugins = [
  store => {
    store.subscribe((mutation, state) => {
      ...
      if (mutation.type.indexOf(types.SN_WATCHEDLIST) >= 0) { // check if this mutation is user related
        global.db.watchedCollection.setItem('watched-history', state.watched.itemsWatched).catch((reason) => {
          console.error(reason) // it doesn't work on SSR
        }) // populate cache
      }
