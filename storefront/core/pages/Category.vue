      ...
      store.dispatch('category/list', {}).then((categories) => {
        store.dispatch('attribute/list', { // load filter attributes for this specific category
          ...
          store.dispatch('category/single', { key: 'slug', value: route.params.slug }).then((parentCategory) => {
            filterData({ searchProductQuery: baseFilterQuery(defaultFilters, parentCategory), populateAggregations: true, store: store, route: route, current: 0, perPage: 18, filters: defaultFilters }).then((subloaders) => {
