...
function commandStatic() {
  Promise.all(
   [
    new Promise(commandCmsBlocks),
    new Promise(commandCmsPages)
   ]).catch(function (err) {
     logger.error(err);
     process.exit(1)
   });
 ...
 * Re-index cmsBlocks
 */
function commandCmsBlocks(next, reject) {
  let adapter = factory.getAdapter(cli.options.adapter, 'cmsblock');
  let tsk = new Date().getTime();
 ...
 * Re-index cmsPages
 */
function commandCmsPages(next, reject) {
  let adapter = factory.getAdapter(cli.options.adapter, 'cmspage');
  let tsk = new Date().getTime();
...
* Sync cms blocks
*/
cli.command('cmsblocks', function () {
  commandCmsBlocks();
});
/**
 * Sync cms pages
 */
cli.command('cmspages', function () {
  commandCmsPages();
});
