...
export function quickSearchByQuery ({ query, start = 0, size = 50, entityType = 'product', sort = '' }) {
  ...
  if (entityType === 'category') {
    index = config.elasticsearch.indexCategory
    entityType = 'pim_catalog_category'
  }
  if (entityType === 'product' || entityType === 'attribute') {
    index = config.elasticsearch.indexProduct
    entityType = 'pim_catalog_product'
  }

  return new Promise((resolve, reject) => {
    const esQuery = {
      index: index, // TODO: add grouped prodduct and bundled product support
      type: entityType,
      body: query,
      size: size,
      from: start,
